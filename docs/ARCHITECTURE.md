...
| 날짜 | 사건 |
|---|---|
| 2026-08-02 | **상품 전체 이미지 보존 + 안전한 대표 이미지 갱신** — migration 102에서 `products.images` 및 `apply_product_image_selections`의 10장 제한을 제거했다. migration 103에서 대표 이미지가 실제로 바뀐 상품은 기존 `product_embeddings`만 삭제해 새 이미지 기준으로 재생성하고, `product_features`는 보존하도록 조정했다. Apple Vision 선택은 macOS 로컬 수동 작업으로 유지한다. |
