...
> 최종 업데이트: 2026-08-06 (migration 105 — 브랜드 설명을 `brand_nodes.description text`로 분리하고 wiki 설명 키 제거)
...
- `src/app/admin/brand-nodes/brand-node-detail.tsx` — 브랜드 상세 Sheet. products 대표이미지 + 유사 브랜드 + price_min/max_usd + 일반 `description` 및 **Wiki 섹션** (view/edit, SPEC-BRAND-WIKI-001 M2)
...
- `src/app/api/admin/brand-nodes/[id]/wiki/route.ts` — `PATCH` (`description` 일반 컬럼 update + wiki jsonb merge-update, SPEC-BRAND-WIKI-001 M2). 도메인 실체: `src/domains/admin-tools/brand-management/brand-nodes__id__wiki.route.ts`
