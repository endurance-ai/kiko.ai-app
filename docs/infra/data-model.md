...
| **상품** | `products` | 004 + 005 + 006 + 011 + 027 + **070** + **107** | 크롤로 들어온 모든 SKU. **070: id uuid→bigserial 전환 (2026-05-18)**. `gender` 는 크롤러 write-path의 단일 출처이며, 크롤러 색상 컬럼 `color` 는 **107 에서 DROP**. 색상 정본은 `product_features.feature_metadata->>'primary_color'`. |
