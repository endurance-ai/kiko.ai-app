...
| **099** | **`products.color` NOT NULL 해제 (2026-08-01)** — 096(gender CHECK 해제)의 color 판. 2026-07-29 VLM 이관으로 크롤러가 color 를 만들지 않게 됐는데 컬럼은 NOT NULL + DEFAULT 없음이라, **신규상품 워커와 온보딩 import 가 구조적으로 INSERT 불가** 상태였다(실측: products 마지막 적재 07-29, 이후 0행 / 워커 실패 210건 전부 `null value in column "color"`). UPDATE 경로는 color 를 안 건드려 갱신 지표에는 안 보이던 고장. 컬럼은 유지 — 기존 154,403행 값은 `analyze-products.ts` 가 VLM 힌트로 쓴다(null 안전). 색의 단일 출처는 `product_features.feature_metadata->>'primary_color'`. |
