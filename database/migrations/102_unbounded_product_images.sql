-- Preserve every collected product image and make representative-image changes
-- invalidate image-derived search assets.

BEGIN;

ALTER TABLE products
  ALTER COLUMN image_selection_candidate_count TYPE integer,
  ...
  ADD CONSTRAINT products_image_selection_candidate_count_check
    CHECK (
      ...
      OR image_selection_candidate_count > 0
    );
...
COMMENT ON FUNCTION apply_product_image_selections(jsonb) IS
  'Applies unbounded image arrays and invalidates embeddings/features when the representative image changes.';
